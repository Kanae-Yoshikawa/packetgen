...
typedef struct _UDP UDP;	// ここで用いる構造体は既に定義済みのもの．それをrename する
		...
		uint16_t srcMAC1, uint32_t srcMAC2, uint32_t vlanTag, uint16_t type, int32_t pValue)
{
	...
	headerSize = sizeof(UDP) - sizeof(unsigned char *);
	/* errorかも?? -> error 回避sampleみろ
	...
	packet->UdpChecksum = htons(0x0000);      //UDP checksum   後で直す
